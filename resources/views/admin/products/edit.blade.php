<x-app-layout title="Editar producto">
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex flex-col gap-1">
                ...
                <h1 class="text-3xl font-semibold text-white">Editar producto</h1>
                <p class="text-sm text-slate-300">{{ $product->title }}</p>
            </div>
    ...
    <div class="mx-auto max-w-5xl px-6 py-10 sm:px-10 lg:px-12">
        ...
        <form method="POST" action="{{ route('admin.products.destroy', $product) }}" onsubmit="return confirm('¿Seguro que deseas eliminar este producto?');" class="mt-6 flex justify-end">
            @csrf
            @method('DELETE')
